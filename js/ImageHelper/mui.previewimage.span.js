// <span 
//	style="background:url(imgSrcMin)" 
//	data-preview-src="imgSrcMax" 
//	data-preview-group="item.id" 
//	data-content="item.info"></span>

var $previewImageShow, $previewImageHide;

(function($, window) {
	var template = '<div id="{{id}}" class="mui-slider mui-preview-image mui-fullscreen"><div class="mui-preview-header">{{header}}</div><div class="mui-slider-img-content">{{content}}</div><div class="mui-slider-img-delete">{{imgurl}}</div><div class="mui-slider-group"></div><div class="mui-preview-footer mui-hidden">{{footer}}</div><div class="mui-preview-loading"><span class="mui-spinner mui-spinner-white"></span></div></div>';
	...
	proto.init = function() {
		...
		this.deletes = this.element.querySelector('.mui-slider-img-delete');
		if(options.footer) {
	...
	proto.initEvent = function() {
		...
		$(document.body).on('tap', '*[data-preview-src]', function() {
			...
			if(self.isAnimationing()) return false;

			// 判断是否显示删除
			var previewGroup = jQuery(this).attr('data-preview-group') - 0;

			if(previewGroup == 1 && UpImage.upImg.option.delete) {
				UpImage.upImg.btn.delete.show();
			} else {
				UpImage.upImg.btn.delete.hide();
			}
			...
			setTimeout(function() {
				...
				if(!mui.os.android) {
					...
					plus.navigator.setStatusBarStyle('UIStatusBarStyleBlackOpaque'); //设置状态栏颜色白底黑字
				}
		...
		var laterCloseEvent = function() {
			...
			!laterClose && (laterClose = $.later(function() {
				...
				self.deletes.removeEventListener('tap', laterCloseEvent);
				self.close();
				...
				if(!mui.os.android) {
					...
					plus.navigator.setStatusBarStyle('UIStatusBarStyleDefault'); //设置状态栏颜色白底黑字
				}
		...
		this.element.addEventListener('webkitAnimationEnd', function() {
			if(self.element.classList.contains($.className('preview-out'))) { //close
				...
				self.deletes.addEventListener('tap', laterCloseEvent);
			}
	...
	proto._initImgData = function(itemData, imgEl) {
		if(!itemData.sWidth) {
			...
			itemData.sScale = itemData.sScale > 1 ? 0.977 : itemData.sScale;
		}
	...
	proto.refresh = function(index, groupArray) {
		...
		for(var i = 0; from < to; from++, i++) {
			...
			var __MuiImgZoom = document.getElementById('__MUI_PREVIEWIMAGE');

			__MuiImgZoom.getElementsByClassName('mui-slider-img-content')[0].innerHTML = "<span>" + itemData.el.getAttribute('data-content') || '' + "</span>";
			__MuiImgZoom.getElementsByClassName('mui-slider-img-delete')[0].innerHTML = '<i class="iconfont icon-shanchu"></i>';
