...
var barHeight = 0;
			...
			if(ms && ms.length >= 3) {
				if(!localStorage.barHeight) {
					barHeight = parseFloat(ms[2]);
					localStorage.headHeight = ((barHeight - 0) + 44) + "px";
					localStorage.barHeight = barHeight + "px"
				}
				var docHeader = jQuery("header");
				if(docHeader.length) {
					if(mui.os.android) {
						docHeader.css("margin-top", localStorage.barHeight);
						jQuery(".mui-content").css("margin-top", localStorage.barHeight);
						docHeader.before('<div class="headerTop" style=" height: ' + localStorage.barHeight + '"></div>')
					} else {
						docHeader.css("height", localStorage.headHeight);
						docHeader.css("padding-top", localStorage.barHeight);
						jQuery(".mui-content").css("padding-top", localStorage.headHeight)
					}
				}
			};
...
var Reg = {
	lable: new RegExp("\\[([^\\[\\]]*?)\\]", 'igm')
};


function OpenView(Url,Id,Obj,anShow)
{
	anShow=anShow||"";
	Obj=Obj||{};
	
	if(anShow==""){anShow="slide-in-right"}
	else{anShow="slide-in-"+anShow+""}
	mui.openWindow(
		{
			url:Url,
			id:Id,
...
function OpenNaveView(Url, Id, Obj, anShow, title, splitLine,buttons) {
	
	if(anShow == undefined || anShow == null || anShow == '') {
		
		anShow = "slide-in-right"
		
	} else {
		
		anShow = "slide-in-" + anShow + ""
	...
	buttons.push({
		...
		fontSize: '24px',
		onclick: function() {
			mui.fire(plus.webview.getWebviewById(Id), 'autoBackButton', {});
		}
	});
	...
	mui.openWindow({
		...
		styles: {
			titleNView: {
				...
				buttons: mui.os.android ? [] : buttons,
				splitLine: {
